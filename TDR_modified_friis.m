function [ ] = TDR_modified_friis( moisture, sand, clay )
...
distance = 0:0.000001:6;

RI_dry = 1.634 - (0.539e-2)*clay + (0.2748e-4)*clay*clay^2; %RI_dry is the refractive index of dry soil
NAC_dry = 0.03952 - (0.04038e-2)*clay; %NAC_dry is the Normalized Attenution Coefficient known as k for dry soil 
...
epsilon_real_CDC_mbsdm = (RI_mbsdm*RI_mbsdm) - (NAC_mbsdm*NAC_mbsdm)  %Real part of the mixing CDC
...
%--------------------------------------------------------------------------
Loss_Path_proposed_peplinski_reflect = reflect_attenuation_peplinski + 20*log10(distance_boite1 * distance_boite2) - 288.8 + 40*log10(frequency_dielectric) + 20*log10(distance) + 20*log10(beta_peplinski) + (8.68 * alph_peplinski * distance);
Loss_Path_proposed_peplinski = 20*log10(distance_boite1 * distance_boite2) - 288.8 + 40*log10(frequency_dielectric) + 20*log10(distance) + 20*log10(beta_peplinski) + (8.68 * alph_peplinski * distance);
...
Loss_Path_proposed_mbsdm_3 = 20*log10(distance_boite1 * distance_boite2 * 3 * beta_mbsdm) - 288.8 + 40*log10(frequency_dielectric) + (8.68 * alph_mbsdm * 3)
%Loss_Path_proposed_mbsdm_2 = Loss_Path_proposed_mbsdm_3 + 20*log10(distance/3);
...
Loss_Path_proposed_tmdm_reflect = reflect_attenuation_tmdm + 20*log10(distance_boite1 * distance_boite2) - 288.8 + 40*log10(frequency_dielectric) + 20*log10(distance) + 20*log10(beta_tmdm) + (8.68 * alph_tmdm * distance);
Loss_Path_proposed_tmdm = 20*log10(distance_boite1 * distance_boite2) - 288.8 + 40*log10(frequency_dielectric) + 20*log10(distance) + 20*log10(beta_tmdm) + (8.68 * alph_tmdm * distance);
%--------------------------------------------------------------------------
...
%     p6 = plot(distance,Loss_Path_modified_friis_mbsdm, 'LineWidth',1);
%     p3 = plot(distance,Loss_Path_proposed_mbsdm, 'LineWidth',1);
%     p4 = plot(distance,Loss_Path_proposed_mbsdm_reflect,'LineWidth',1);
%     
% %     plot(1 ,[106 107 108], 'ko', 'LineWidth',1);
...
% 
% % Moisture of 20-21 % distance 1m , 2m between nodes
% %     plot(1 ,[117 116 106 109], 'ko', 'LineWidth',1);
% %     plot(2 ,[132 125 131 130 120 123 124 126], 'ko', 'LineWidth',1);
% %     %plot(4 ,[114 113 117 119], 'ko', 'LineWidth',1);
% %     p5 = plot(1 ,108, 'ko', 'LineWidth',1);
% %     %plot(5 ,[137 131], 'ko', 'LineWidth',1);
% %     plot(5 ,131, 'ko', 'LineWidth',1);
%     
% %Moisture of 14% distance 1m
%     %plot(1 ,[114 113 105 106 104], 'ro', 'LineWidth',1);
% %     plot(1 ,[106 105 99 98 97], 'ko', 'LineWidth',1);
% %     p5 = plot(1 ,108, 'ko', 'LineWidth',1);
% 
% %Moisture of 41% distance 5m and depth 30cm to 30cm
% 
% %     plot(5 ,[147 148], 'ko', 'LineWidth',1);
% %     plot(5 ,[136 135], 'ko', 'LineWidth',1);
% 
% grid on
%     grid minor
%     legend([p1 p2 p6 p3 p4],{' Modified Friis ', ' NC Modified Friis ', ' MBSDM Modified Friis ', ' Proposed path loss approach ', ' Proposed path loss reflection '},'Location','northwest')
%     ylabel('Path Loss (dB)','FontWeight','bold')
%     xlabel('Distance (m)','FontWeight','bold')
...
rssi_proposed = transmitted_power + 2*antenna_gain - Loss_Path_proposed_mbsdm;
...
figure(7)
    ...
    p5 = plot(distance,rssi_proposed,'g', 'LineWidth',1);
    
    plot(1 ,[-80 -95 -94 -84 -87], 'ko', 'LineWidth',1);
    plot(2 ,[-110 -109 -108 -103 -98 -101 -102 -104 -90], 'ko', 'LineWidth',1);
    plot(3 ,[-96 -88], 'ko', 'LineWidth',1);
    %plot(3.1 ,-90, 'ko', 'LineWidth',1);
    plot(4 ,[-91 -95 -97], 'ko', 'LineWidth',1);
    %plot(4.1 ,-93, 'ko', 'LineWidth',1);
    plot(5 ,[-109 -108 -115 -120], 'ko', 'LineWidth',1);
    %plot(5.1 ,-118, 'ko', 'LineWidth',1);
    p4 = plot(5 ,-115, 'ko', 'LineWidth',1);
       
    grid on
    grid minor
    legend([p1 p2 p5 p4],{' Conv. Modified Friis ', ' NC Modified Friis ', ' WUSN-PLM ', ' Measured RSSI'},'Location','northwest')
    ylabel('RSSI (dBm)','FontWeight','bold')
    xlabel('Distance (m)','FontWeight','bold')
    title('Power Received (Sandy clay #1)','FontWeight','bold', 'FontSize', 13)
    hold off
% 
% figure(8)
%     p1 = plot(distance,Loss_Path_modified_friis_peplinski,'-', 'LineWidth',1);
...
% 
% % Sandy clay #1 dry soil 
% plot(5 ,[43.8 21.18 72.78 50.64], 'kx');
%      plot(10 ,[49.93 27.31 95.76 73.62], 'kx');
%      plot(15 ,[53.56 30.94 114.8 92.69], 'kx');
%      plot(20 ,[56.17 33.56 132.9], 'kx');
%     p6 = plot(20 ,110.7, 'kx'); 
%     
% 
% %     plot([3 4 5] ,140, 'kx');
...
%     grid on
%     grid minor
%     legend([p1 p2 p4 p5 p6],{' Conv. Modified Friis ', ' NC Modified Friis ', ' Proposed WUSN-PLM #1 ', ' Proposed WUSN-PLM #2 ', ' Evaluated points'},'Location','northwest')
%     ylabel('Path Loss (dB)','FontWeight','bold')
%     xlabel('Distance (m)','FontWeight','bold')
